...
pub async fn point_msm(wgsl_source: &str, point_bytes: Vec<&[u8]>, scalar_bytes: Vec<&[u8]>) -> Vec<u16> {
    let (device, queue) = setup_webgpu().await;
    let shader_module = device.create_shader_module(wgpu::ShaderModuleDescriptor {
        label: Some("Point Shader"),
        source: wgpu::ShaderSource::Wgsl(wgsl_source.into()),
    ...
    let points = point_bytes.iter().map(|b| device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
        label: None,
        contents: b,
        usage: wgpu::BufferUsages::STORAGE | wgpu::BufferUsages::COPY_DST,
    })).collect::<Vec<_>>();

    let scalars = scalar_bytes.iter().map(|b| device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
        label: None,
        contents: b,
        usage: wgpu::BufferUsages::STORAGE | wgpu::BufferUsages::COPY_DST,
    })).collect::<Vec<_>>();
    ...
    let msm_len_buffer = device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
        ...
        contents: &(point_bytes.len() as u32).to_le_bytes(),
        usage: wgpu::BufferUsages::STORAGE | wgpu::BufferUsages::COPY_DST,
    });
